...
function createChatButton(options = {}) {
  ...
  if (options.buttonContainerId) {
    ...
    // 否则尝试获取FineReport工具栏
    buttonContainer = document.querySelector(".fr-toolbar") || document.querySelector(".fr-btn-panel");
  }

  // 如果没有找到容器，则创建一个新的容器并添加到报表容器中
  if (!buttonContainer) {
    const reportContainer = document.querySelector(".fr-designer") || document.querySelector(".fr-viewer") || document.body;
