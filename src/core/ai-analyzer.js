...
import { showLoadingIndicator, hideLoadingIndicator, updateLoadingProgress } from "../ui/loading-indicator.js";
 ...
 * @returns {string} 缓存键
 */
function generateCacheKey(prompt, apiConfig) {
  // 使用prompt和关键配置参数生成缓存键
  const keyParts = [
    prompt,
    apiConfig.model,
 ...
 */
async function analyzeWithAI(prompt, options = {}) {
  // 合并默认配置和用户配置
  ...
  try {
    ...
    if (config.useCache) {
      const cacheKey = generateCacheKey(prompt, apiConfig);
    ...
    } else if (error.message.includes("network") || error.message.includes("连接") || error.message.includes("connection")) {
 ...
 */
async function streamAnalyzeWithAI(prompt, options = {}, onChunk, onComplete) {
    ...
    while (fallbackAttempts <= config.maxFallbackAttempts) {
      ...
      while (retryCount <= config.maxRetries) {
        try {
          ...
          if (shouldRetry && retryCount < config.maxRetries) {
            retryCount++;
            console.warn(`流式AI请求失败 (尝试 ${retryCount}/${config.maxRetries}): ${error.message}`);
          } else {
            // 更新模型性能统计
            requestStatus.modelPerformance[apiConfig.model].failures++;
...
function simulateStreamFromText(text) {
  ...
  while (start < text.length) {
    // 随机化块大小，使其看起来更自然
    const size = Math.max(1, Math.floor(avgChunkSize + (Math.random() * avgChunkSize - avgChunkSize/2)));
