 ...
 */
import { getPromptTemplates } from "../config/prompt.js";
...
function buildBasicAnalysisPrompt(reportData) {
  ...
  let prompt = templates.basicAnalysis.header;

  // 添加表格数据
  if (reportData.tableData && reportData.tableData.length > 0) {
    prompt += templates.basicAnalysis.tableSection;
  ...
  if (reportData.chartData && reportData.chartData.length > 0) {
    prompt += templates.basicAnalysis.chartSection;
    prompt += JSON.stringify(reportData.chartData, null, 2);
    prompt += "\n\n";
  }

  // 添加分析要求
  prompt += templates.basicAnalysis.requirements;
...
export { buildBasicAnalysisPrompt, buildCustomAnalysisPrompt };
